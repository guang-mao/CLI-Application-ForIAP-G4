...
static uint8_t cli_update(void *para, uint8_t len);
...
const char CLI_Cmd_Update[] =
    "[update]\r\n"
    " * Firmware update for MCU\r\n"
    "\r\n";
...
void cli_init(uint32_t baud)
{
    ...
    PRINTF("    coder: Howard                 \r\n\r\n");
    PRINTF("    Email: m26402000@gmail.com    \r\n\r\n");
        ...
        if(Handle.len < HANDLE_LEN) {  /* check the buffer */
            ...
            if(TRUE == QUEUE_OUT((*rx_buff), Handle.buff[Handle.len])) {
                ...
                if ((0 == key) && (i < Handle.len)) {
                    ...
                    for (; i < Handle.len; i++)
                    {
                        LL_USART_TransmitData8(DEBUG_USARTx, Handle.buff[i]);
                    }
#if CLI_HISTORY
                }
#endif  /* CLI_HISTORY */
